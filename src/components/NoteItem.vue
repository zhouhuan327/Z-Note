	...
	<el-card class="box-card" :body-style="{ minHeight: '50px',maxHeight:'90px' }">
		<div slot="header">
			<el-row class="row-title" type="flex">
				...
				<el-col style="text-align: right" :lg="8" :md="8" :sm="8" :xs="8">
		...
		<div ref="text" class="content">{{FilterContent}}</div>
	</el-card>
</template>
...
export default class NoteItem extends Vue {
	...
	get getIconName() {
		const type = this.$store.state.actionHelper
			.getCategoryName(this.note.categoryId)
			.toLowerCase()
		return `icon-${type}`
	}
}
